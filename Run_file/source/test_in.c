...
//Buffer에 추가 하는 부분 char *으로 받고 추가적인 부분 ex) %s, "12" 이런 부분들 가변적으로 받을 수 있도록 설계
int appendBuffer(const char *str, ...) {
    //va_start(args, str)로 가변 인자를 초기화하고, va_end(args)로 가변 인자 처리를 종료합니다.
    va_list args;
    va_start(args, str);

    //가변 인자를 받는 경우에는 snprintf가 아닌 vsnprintf를 사용하여야한다.
    //char에 담는 쓰는 역할을 하며 반환값으로 얼마나 썼는지 (안녕세요 를 썼다고 하면 5 가 반환값이다.)
    size_t written = vsnprintf(pBuffer + bufferOffset, nBufferSize - bufferOffset, str, args);

    va_end(args);
    //

    if(written >= 0 && written < nBufferSize - bufferOffset) {
        bufferOffset += written;
    } else {
        bufferOffset = nBufferSize - 1;
...
int writeToFile(const char *fileName) {
    ...
    size_t written = fwrite(pBuffer, 1, bufferOffset, pFile);
    fclose(pFile);
        ...
        for (idxloop = 1; idxloop <= nidx; idxloop++)
        {
            ...
            if (idx > 0)
            {
                ...
                for (subidxloop = 1; subidxloop <= subidx; subidxloop++)
                {
                    ...
                    //print("  [0x%4.4X.%1d] 0x%4.4X:0x%2.2X 0x%2.2X", abs_offset, abs_bit, obj_idx, obj_subidx, bitlen);
                    appendBuffer("  [0x%4.4X.%1d] 0x%4.4X:0x%2.2X 0x%2.2X", abs_offset, abs_bit, obj_idx, obj_subidx, bitlen);
...
int si_map_sdo(int slave)
{
    ...
    //print("PDO mapping according to CoE :\n");
    appendBuffer("PDO mapping according to CoE :\n");
    SMt_bug_add = 0;
    outputs_bo = 0;
    inputs_bo = 0;
        ...
        for (iSM = 2 ; iSM <= nSM ; iSM++)
        {
            ...
            if (wkc > 0)
            {
                if((iSM == 2) && (tSM == 2)) // SM2 has type 2 == mailbox out, this is a bug in the slave!
                {
                    ...
                    //print("Activated SM type workaround, possible incorrect mapping.\n");
                    appendBuffer("Activated SM type workaround, possible incorrect mapping.\n");
                }
                if(tSM)
    ...
    if (PDO->Startpos > 0)
    {
        ...
        do
        {
            ...
            if (PDO->SyncM[PDO->nPDO] < EC_MAXSM) /* active and in range SM? */
            {
                ...
                if (t) {
                  //print("  SM%1d RXPDO 0x%4.4X %s\n", PDO->SyncM[PDO->nPDO], PDO->Index[PDO->nPDO], str_name);
                  appendBuffer("  SM%1d RXPDO 0x%4.4X %s\n", PDO->SyncM[PDO->nPDO], PDO->Index[PDO->nPDO], str_name);
                }
                else {
                  //print("  SM%1d TXPDO 0x%4.4X %s\n", PDO->SyncM[PDO->nPDO], PDO->Index[PDO->nPDO], str_name);
                  appendBuffer("  SM%1d TXPDO 0x%4.4X %s\n", PDO->SyncM[PDO->nPDO], PDO->Index[PDO->nPDO], str_name);
                }
                //print("     addr b   index: sub bitl data_type    name\n");
                appendBuffer("     addr b   index: sub bitl data_type    name\n");
...
int si_map_sii(int slave)
{
    ...
    //print("PDO mapping according to SII :\n");
    appendBuffer("PDO mapping according to SII :\n");
...
void  si_sdo(int cnt)
{
    ...
    {
        //print(" CoE Object Description found, %d entries.\n",ODlist.Entries);
        appendBuffer(" CoE Object Description found, %d entries.\n",ODlist.Entries);
        for( i = 0 ; i < ODlist.Entries ; i++)
        {
            ...
            if (ODlist.ObjectCode[i] == OTYPE_VAR)
            {
                ...
                appendBuffer("0x%04x      %-40s      [%s]\n", ODlist.Index[i], name,
                       otype2string(ODlist.ObjectCode[i]));
            }
            else
            {
                //print("0x%04x      %-40s      [%s  maxsub(0x%02x / %d)]\n",
                       //ODlist.Index[i], name, otype2string(ODlist.ObjectCode[i]),
                       //ODlist.MaxSub[i], ODlist.MaxSub[i]);
                appendBuffer("0x%04x      %-40s      [%s  maxsub(0x%02x / %d)]\n",
                       ODlist.Index[i], name, otype2string(ODlist.ObjectCode[i]),
                       ODlist.MaxSub[i], ODlist.MaxSub[i]);
            }
            ...
            for( j = 0 ; j < max_sub+1 ; j++)
            {
                if ((OElist.DataType[j] > 0) && (OElist.BitLength[j] > 0))
                {
                    ...
                    //print("    0x%02x      %-40s      [%-16s %6s]      ", j, name,
                           //dtype2string(OElist.DataType[j], OElist.BitLength[j]),
                           //access2string(OElist.ObjAccess[j]));
                    appendBuffer("    0x%02x      %-40s      [%-16s %6s]      ", j, name,
                           dtype2string(OElist.DataType[j], OElist.BitLength[j]),
                           access2string(OElist.ObjAccess[j]));
                    if ((OElist.ObjAccess[j] & 0x0007))
                    {
                        //print("%s", SDO2string(cnt, ODlist.Index[i], j, OElist.DataType[j]));
                        appendBuffer("%s", SDO2string(cnt, ODlist.Index[i], j, OElist.DataType[j]));
                    }
                    //print("\n");
                    appendBuffer("\n");
                }
...
void slaveinfo(char *ifname)
{
   ...
   //print("Starting slaveinfo\n");

   /* initialise SOEM, bind socket to ifname */
   if (ec_init(ifname))
   {
      //print("ec_init on %s succeeded.\n",ifname);
      appendBuffer("ec_init on %s succeeded.\n",ifname);
      /* find and auto-config slaves */
      if ( ec_config(FALSE, &IOmap) > 0 )
      {
         ec_configdc();
         while(EcatError) printf("%s", ec_elist2string());
         //print("%d slaves found and configured.\n",ec_slavecount);
         appendBuffer("%d slaves found and configured.\n",ec_slavecount);
         expectedWKC = (ec_group[0].outputsWKC * 2) + ec_group[0].inputsWKC;
         //print("Calculated workcounter %d\n", expectedWKC);
         appendBuffer("Calculated workcounter %d\n", expectedWKC);
         /* wait for all slaves to reach SAFE_OP state */
         ec_statecheck(0, EC_STATE_SAFE_OP,  EC_TIMEOUTSTATE * 3);
         if (ec_slave[0].state != EC_STATE_SAFE_OP )
         {
            //print("Not all slaves reached safe operational state.\n");
            appendBuffer("Not all slaves reached safe operational state.\n");
            ec_readstate();
            for(i = 1; i<=ec_slavecount ; i++)
            {
               if(ec_slave[i].state != EC_STATE_SAFE_OP)
               {
                  //print("Slave %d State=%2x StatusCode=%4x : %s\n",
                     //i, ec_slave[i].state, ec_slave[i].ALstatuscode, ec_ALstatuscode2string(ec_slave[i].ALstatuscode));
                    appendBuffer("Slave %d State=%2x StatusCode=%4x : %s\n",
                     i, ec_slave[i].state, ec_slave[i].ALstatuscode, ec_ALstatuscode2string(ec_slave[i].ALstatuscode));
               }
            }
         }
         ...
         for( cnt = 1 ; cnt <= ec_slavecount ; cnt++)
         {
            ...
            if (ec_slave[cnt].hasdc) {
                //print(" DCParentport:%d\n", ec_slave[cnt].parentport);
                appendBuffer(" DCParentport:%d\n", ec_slave[cnt].parentport);
            }
            //print(" Activeports:%d.%d.%d.%d\n", (ec_slave[cnt].activeports & 0x01) > 0 ,
                                        // (ec_slave[cnt].activeports & 0x02) > 0 ,
                                         //(ec_slave[cnt].activeports & 0x04) > 0 ,
                                         //(ec_slave[cnt].activeports & 0x08) > 0 );
            appendBuffer(" Activeports:%d.%d.%d.%d\n", (ec_slave[cnt].activeports & 0x01) > 0 ,
                                         (ec_slave[cnt].activeports & 0x02) > 0 ,
                                         (ec_slave[cnt].activeports & 0x04) > 0 ,
                                         (ec_slave[cnt].activeports & 0x08) > 0 );
            //print(" Configured address: %4.4x\n", ec_slave[cnt].configadr);
            appendBuffer(" Configured address: %4.4x\n", ec_slave[cnt].configadr);
            //print(" Man: %8.8x ID: %8.8x Rev: %8.8x\n", (int)ec_slave[cnt].eep_man, (int)ec_slave[cnt].eep_id, (int)ec_slave[cnt].eep_rev);
            appendBuffer(" Man: %8.8x ID: %8.8x Rev: %8.8x\n", (int)ec_slave[cnt].eep_man, (int)ec_slave[cnt].eep_id, (int)ec_slave[cnt].eep_rev);
            for(nSM = 0 ; nSM < EC_MAXSM ; nSM++)
            {
               if(ec_slave[cnt].SM[nSM].StartAddr > 0)
                  //print(" SM%1d A:%4.4x L:%4d F:%8.8x Type:%d\n",nSM, etohs(ec_slave[cnt].SM[nSM].StartAddr), etohs(ec_slave[cnt].SM[nSM].SMlength),
                         //etohl(ec_slave[cnt].SM[nSM].SMflags), ec_slave[cnt].SMtype[nSM]);
                    appendBuffer(" SM%1d A:%4.4x L:%4d F:%8.8x Type:%d\n",nSM, etohs(ec_slave[cnt].SM[nSM].StartAddr), etohs(ec_slave[cnt].SM[nSM].SMlength),
                         etohl(ec_slave[cnt].SM[nSM].SMflags), ec_slave[cnt].SMtype[nSM]);
            }
            for(j = 0 ; j < ec_slave[cnt].FMMUunused ; j++)
            {
               //print(" FMMU%1d Ls:%8.8x Ll:%4d Lsb:%d Leb:%d Ps:%4.4x Psb:%d Ty:%2.2x Act:%2.2x\n", j,
                ...
                appendBuffer(" FMMU%1d Ls:%8.8x Ll:%4d Lsb:%d Leb:%d Ps:%4.4x Psb:%d Ty:%2.2x Act:%2.2x\n", j,
                       etohl(ec_slave[cnt].FMMU[j].LogStart), etohs(ec_slave[cnt].FMMU[j].LogLength), ec_slave[cnt].FMMU[j].LogStartbit,
                       ec_slave[cnt].FMMU[j].LogEndbit, etohs(ec_slave[cnt].FMMU[j].PhysStart), ec_slave[cnt].FMMU[j].PhysStartBit,
                       ec_slave[cnt].FMMU[j].FMMUtype, ec_slave[cnt].FMMU[j].FMMUactive);
            }
            //print(" FMMUfunc 0:%d 1:%d 2:%d 3:%d\n",
                     //ec_slave[cnt].FMMU0func, ec_slave[cnt].FMMU1func, ec_slave[cnt].FMMU2func, ec_slave[cnt].FMMU3func);
            appendBuffer(" FMMUfunc 0:%d 1:%d 2:%d 3:%d\n",
                     ec_slave[cnt].FMMU0func, ec_slave[cnt].FMMU1func, ec_slave[cnt].FMMU2func, ec_slave[cnt].FMMU3func);
            //print(" MBX length wr: %d rd: %d MBX protocols : %2.2x\n", ec_slave[cnt].mbx_l, ec_slave[cnt].mbx_rl, ec_slave[cnt].mbx_proto);
            appendBuffer(" MBX length wr: %d rd: %d MBX protocols : %2.2x\n", ec_slave[cnt].mbx_l, ec_slave[cnt].mbx_rl, ec_slave[cnt].mbx_proto);
            ...
            //print(" CoE details: %2.2x FoE details: %2.2x EoE details: %2.2x SoE details: %2.2x\n",
                    //ec_slave[cnt].CoEdetails, ec_slave[cnt].FoEdetails, ec_slave[cnt].EoEdetails, ec_slave[cnt].SoEdetails);
            appendBuffer(" CoE details: %2.2x FoE details: %2.2x EoE details: %2.2x SoE details: %2.2x\n",
                    ec_slave[cnt].CoEdetails, ec_slave[cnt].FoEdetails, ec_slave[cnt].EoEdetails, ec_slave[cnt].SoEdetails);
            //print(" Ebus current: %d[mA]\n only LRD/LWR:%d\n",
                    //ec_slave[cnt].Ebuscurrent, ec_slave[cnt].blockLRW);
            appendBuffer(" Ebus current: %d[mA]\n only LRD/LWR:%d\n",
                    ec_slave[cnt].Ebuscurrent, ec_slave[cnt].blockLRW);
      ...
      //print("End slaveinfo, close socket\n");
...
int main(int argc, char *argv[])
{
   ...
   //print("SOEM (Simple Open EtherCAT Master)\nSlaveinfo\n");
   ...
   {
      //print("Usage: slaveinfo ifname [options]\nifname = eth0 for example\nOptions :\n -sdo : print SDO info\n -map : print mapping\n");

      //print ("Available adapters\n");
      appendBuffer("Available adapters\n");
      adapter = ec_find_adapters ();
      while (adapter != NULL)
      {
         //print ("Description : %s, Device to use for wpcap: %s\n", adapter->desc,adapter->name);
         appendBuffer("Description : %s, Device to use for wpcap: %s\n", adapter->desc,adapter->name);
         adapter = adapter->next;
      }
      ec_free_adapters(adapter);

      writeToFile("socketINFO.txt");
      free(pBuffer);
      //print("End program\n");
      return (0);
   }
